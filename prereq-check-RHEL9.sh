...
# Modified 2025 to support RHEL 9.x and OpenJDK 17
#
...
#
# CHANGES IN v1.5.6:
# - Added support for RHEL/CentOS 9.x detection and checks
# - Added support for OpenJDK 17 validation
# - Updated Java path candidates for RHEL 9
# - Extended systemd service checks for RHEL 9
#
# DISCLAIMER
#
...
function check_java() {
    ...
    local OPENJAVA17_HOME_CANDIDATES=(
      ...
      '/usr/lib64/jvm/java-1.17.0-openjdk'
    )
    ...
    for candidate_regex in "${JAVA_HOME_CANDIDATES[@]}"; do
        ...
        for candidate in $(ls -rvd ${candidate_regex}* 2>/dev/null); do
            if [ -x "$candidate/bin/java" ]; then
                ...
                JDK_VERSION_REGEX='(1|11|17)\.([0-9])\.([0-9_]*)'
                ...
                if [[ $JDK_VERSION =~ $JDK_VERSION_REGEX ]]; then
                    ...
                    elif [[ ${BASH_REMATCH[1]} -eq 17 ]]; then
...
function check_os() (
    ...
    function check_tuned() {
        # "tuned" service should be disabled on RHEL/CentOS 7.x, 8.x, and 9.x
    ...
    if is_centos_rhel_7 || is_centos_rhel_8 || is_centos_rhel_9; then
        ...
        if [ "${SERVICE_STATE['running']}" = true ]; then
            ...
            state "System: chronyd is NOT running" 1
        fi
    else
        _check_service_is_running 'System' 'chronyd'
        is_chrony_in_sync
